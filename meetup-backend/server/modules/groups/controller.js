...
export const createGroup = async (req, res) => {
    ...
    if(!name) {
        ...
        return res.status(400).json({ error: true, message: 'Name must be at least 5 characters or long'});
    }
    ...
    if(!description) {
        ...
        return res.status(400).json({ error: true, message: 'Description must be at least 10 characters or long'});
    }
...
export const createGroupMeetup = async(req, res) => {
    ...
    if(!title) {
        ...
        return res.status(400).json({ error: true, message: 'Title must be at least 5 characters or long'});
    }
